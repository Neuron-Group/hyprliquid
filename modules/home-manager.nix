{
  ...
  sessionFuzzelConfig =
    pkgs.writeText "hyprliquid-fuzzel.ini"
  ...
  waybarConfig =
    pkgs.writeText "hyprliquid-waybar.jsonc"
    (builtins.replaceStrings
...
in {
  options.wayland.windowManager.hyprland.hyprliquid = {
    ...
    settings = {
      ...
      background_sharing = mkOption {
        type = types.bool;
        default = false;
      };
      watch_system_color_scheme = mkOption {
        type = types.bool;
        default = true;
      };
      effect = mkOption {
      ...
      glass_ior_r = mkOption {
        type = types.float;
        default = 1.02;
      };
      glass_ior_g = mkOption {
        type = types.float;
        default = 1.035;
      };
      glass_ior_b = mkOption {
        type = types.float;
        default = 1.05;
      };
      glass_dispersion = mkOption {
        type = types.bool;
        default = true;
      };
      vdf_map_mode = mkOption {
        type = types.int;
        default = 0;
      };
      vdf_map_update_policy = mkOption {
        type = types.either types.int types.str;
        default = -2;
      };
      vdf_map_debug_mode = mkOption {
        type = types.int;
        default = 0;
      };
      tint_color = mkOption {
      ...
      aero_reflection_map_path = mkOption {
        type = types.str;
        default = "";
      };
      rounding_lua = mkOption {
        type = types.int;
        default = -1;
      };
    };
          ...
          then ''
            windowrule {
                ...
                hyprliquid {
                    effect = liquid_glass
                    rounding_lua = 32
                    tint_color = rgba(10, 88, 205, 0.21)
                }
            }
